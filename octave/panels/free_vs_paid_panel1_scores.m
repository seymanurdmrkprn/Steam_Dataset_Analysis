% Free vs Paid Panel 1: Score Comparison
% Compares positive rating percentages between free and paid games

pkg load statistics;

% Read the score comparison data manually (text with categories)
fid = fopen('../../data/processed/free_vs_paid_scores.csv', 'r');
...
while ~feof(fid)
    ...
    if ischar(line) && ~isempty(line)
        parts = strsplit(line, ',');
        if length(parts) == 2
            score = str2double(parts{1});
            category = strtrim(parts{2});
            
            if strcmp(category, 'Free')
                free_scores = [free_scores; score];
            elseif strcmp(category, 'Paid')
                paid_scores = [paid_scores; score];
            end
        end
    end
end
...
disp(['Free games: ', num2str(length(free_scores))]);
disp(['Paid games: ', num2str(length(paid_scores))]);

% Calculate statistics
stats = [mean(free_scores), mean(paid_scores); 
         median(free_scores), median(paid_scores)];
...
text(1, stats(1,1) + 1.5, sprintf('%.1f%%', stats(1,1)), ...
     'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
     'FontSize', 12, 'FontWeight', 'bold');
text(1+bar_width, stats(2,1) + 1.5, sprintf('%.1f%%', stats(2,1)), ...
     'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
     'FontSize', 12, 'FontWeight', 'bold');
text(2, stats(1,2) + 1.5, sprintf('%.1f%%', stats(1,2)), ...
     'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
     'FontSize', 12, 'FontWeight', 'bold');
text(2+bar_width, stats(2,2) + 1.5, sprintf('%.1f%%', stats(2,2)), ...
     'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', ...
     'FontSize', 12, 'FontWeight', 'bold');
...
% Labels and title
ylabel('Positive Rating (%)', 'FontSize', 17, 'FontWeight', 'bold');
title('Score Comparison: Free vs Paid Games', 'FontSize', 20, 'FontWeight', 'bold');
...
% Add summary
summary_text = sprintf('Paid games have slightly higher ratings on average (%.1f%% vs %.1f%%)', ...
                       mean(paid_scores), mean(free_scores));
...
% Save the figure
print('-dpng', '-r300', '../../outputs/images/free_vs_paid_panel1_scores.png');

disp('Free vs Paid Panel 1: Score comparison saved!');
